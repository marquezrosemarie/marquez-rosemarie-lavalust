...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; } body { margin:0; min-height:100vh; padding:28px 18px; font-family:Georgia,'Times New Roman',serif; color:var(--ink); background:linear-gradient(135deg,#ead6dc,#d8c1c8 52%,#eee0d2); }
        ...
        .access-panel { margin:0 28px 30px; padding:22px 24px; border:1px solid #ffffffb8; border-radius:18px; background:rgba(255,255,255,.34); } .access-panel.is-on { border-color:#d69aaa; background:linear-gradient(120deg,#fff0f3,#f6dce3); } .access-panel.is-off { border-color:#9bcdbd; background:linear-gradient(120deg,#effaf5,#d9eee5); } .access-head { display:flex; justify-content:space-between; align-items:flex-start; gap:18px; } .access-kicker { margin:0 0 7px; color:var(--muted); font-size:.7rem; font-weight:700; letter-spacing:.13em; text-transform:uppercase; } .access-title { margin:0; font-size:1.3rem; } .access-copy { max-width:620px; margin:8px 0 0; color:var(--muted); line-height:1.55; } .status { flex:0 0 auto; display:inline-flex; align-items:center; gap:8px; padding:9px 12px; border-radius:999px; font-size:.76rem; font-weight:700; letter-spacing:.08em; text-transform:uppercase; } .status:before { content:""; width:8px; height:8px; border-radius:50%; background:currentColor; box-shadow:0 0 0 4px currentColor; opacity:.22; } .status-on { color:var(--locked); background:var(--locked-soft); } .status-off { color:var(--safe); background:var(--safe-soft); } .access-actions { display:flex; justify-content:space-between; align-items:center; gap:18px; margin-top:18px; } .blocked-note { margin:0; color:var(--locked); font-size:.88rem; font-weight:700; } .toggle { padding:11px 16px; border:0; border-radius:8px; color:#fff; background:var(--accent-dark); font:inherit; font-size:.88rem; font-weight:700; cursor:pointer; } .toggle:hover { background:var(--accent); } .access-panel.is-off .toggle { color:#174d3e; background:#b6dfcf; } .access-panel.is-off .toggle:hover { background:#a0d2bf; }
...
<body>
    <main class="shell card">
        ...
        <?php if ($middleware_blocked ?? false): ?><p class="notice">Middleware protection intercepted the profile request. Turn protection off below to open the student profile.</p><?php endif; ?>
        <?php $protection_enabled = $middleware_protection_enabled ?? false; ?>
        <section class="access-panel <?= $protection_enabled ? 'is-on' : 'is-off'; ?>">
            <div class="access-head"><div><p class="access-kicker">Middleware control</p><h2 class="access-title">Profile gate is <?= $protection_enabled ? 'active' : 'open'; ?></h2><p class="access-copy"><?= $protection_enabled ? 'The student profile route is currently blocked by middleware.' : 'The student profile route can be opened while middleware protection is off.'; ?></p></div><span class="status <?= $protection_enabled ? 'status-on' : 'status-off'; ?>"><?= $protection_enabled ? 'Protection ON' : 'Protection OFF'; ?></span></div>
            <div class="access-actions"><p class="blocked-note"><?= $protection_enabled ? 'Profile access: blocked' : 'Profile access: allowed'; ?></p><form method="post" action="<?= site_url('student/middleware/toggle'); ?>"><button class="toggle" type="submit"><?= $protection_enabled ? 'Turn protection off' : 'Turn protection on'; ?></button></form></div>
        </section>
        <section class="hero">
