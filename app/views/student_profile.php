...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; } body { margin:0; min-height:100vh; padding:28px 18px; font-family:Georgia,'Times New Roman',serif; color:var(--ink); background:linear-gradient(135deg,#dbe8df,#b7c9c1 52%,#e8dfc9); } .shell { width:min(1100px,100%); margin:auto; }
        .card { background:var(--glass); border:1px solid var(--line); border-radius:28px; box-shadow:0 24px 60px rgba(44,66,56,.18),inset 0 1px 0 #fff; backdrop-filter:blur(18px); overflow:hidden; } .header { display:flex; justify-content:space-between; align-items:center; gap:18px; flex-wrap:wrap; padding:24px 28px; border-bottom:1px solid #ffffff80; } .brand { font-size:1.45rem; font-weight:700; } .brand span { color:var(--accent-dark); }
        ...
        .avatar { width:112px; height:112px; margin-bottom:18px; display:grid; place-items:center; border-radius:18px; color:#fff; font-size:2.5rem; font-weight:700; background:var(--accent-dark); box-shadow:9px 9px 0 #648f7b55; } .profile-title { margin-bottom:12px; font-size:1.45rem; font-weight:700; overflow-wrap:anywhere; } .badge { display:inline-block; padding:7px 10px; border-left:3px solid var(--accent); color:var(--accent-dark); background:#ffffff55; font-size:.73rem; font-weight:700; text-transform:uppercase; }
        h1 { margin:0 0 20px; font-size:clamp(2rem,4vw,2.7rem); } .grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:0 18px; } .item { min-width:0; padding:15px 0; border-bottom:1px solid #ffffff99; } .label { display:block; margin-bottom:7px; color:var(--muted); font-size:.7rem; font-weight:700; letter-spacing:.07em; text-transform:uppercase; } .value { font-weight:700; line-height:1.5; overflow-wrap:anywhere; } .skills { display:flex; flex-wrap:wrap; gap:9px; margin-top:14px; } .skills span { padding:8px 11px; border-radius:5px; color:var(--accent-dark); background:#648f7b26; font-size:.8rem; font-weight:700; }
